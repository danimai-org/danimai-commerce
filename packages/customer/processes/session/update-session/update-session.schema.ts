...
export const UpdateCustomerSessionSchema = Type.Object({
  id: Type.String({
    format: "uuid",
  ...
  customer_id: Type.Optional(
    Type.Union([
      Type.String({
        format: "uuid",
        description: "Associate session with a customer (e.g. after login)",
      }),
      Type.Null(),
    ])
  ),
...
export type UpdateCustomerSessionProcessInput = Static<
  typeof UpdateCustomerSessionSchema
>;

export const UpdateCustomerSessionResponseSchema = Type.Union([
  CustomerSessionResponseSchema,
  Type.Undefined(),
]);
export type UpdateCustomerSessionProcessOutput = Static<
  typeof UpdateCustomerSessionResponseSchema
>;
