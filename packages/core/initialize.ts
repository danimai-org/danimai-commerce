...
function getQueryCaller(): QueryCaller {
  ...
  for (const line of lines) {
    ...
    if (
      ...
      trimmed.includes("kysely")
    )
      continue;

    const pathMatch = trimmed.match(
      /(?:file:\/\/)?([^)\s]+\.(?:ts|js|mts|mjs))(?::\d+)?(?::\d+)?/,
    );
    const rawPath =
      pathMatch && pathMatch[1] ? pathMatch[1].replace(/^file:\/\//, "") : "";
    ...
    if (pkgMatch && pkgMatch[1]) {
      ...
      packageTag =
        ...
        name.slice(1).replace(/-(\w)/g, (_, c: string) => c.toUpperCase());
    ...
    const processName =
      fnName && fnName !== "log" && fnName !== "handler"
        ? fnName
        : fileName || "process";
...
export const getContainer = (): Container => {
  if (!container) {
    throw new Error(
      "Container not initialized. Please call initialize() first.",
    );
  }
  return container;
};
...
export const initialize = ({ db, logger, config }: DanimaiInitialize) => {
  ...
  if (
    container &&
    container.isBound(DANIMAI_INITIALIZED) &&
    container.get(DANIMAI_INITIALIZED)
  ) {
    return;
  }
  ...
  if (!container.isBound(DANIMAI_PASSWORD)) {
    container
      .bind(DANIMAI_PASSWORD)
      .toConstantValue(
        new Password(
          passwordConfig?.algorithm ?? "bcrypt",
          passwordConfig?.cost ?? 10,
        ),
      );
  }
...
export const getProcess = <T extends ProcessContract<any, any>>(
  processName: symbol,
) => {
  ...
  if (!cont.isBound(processName)) {
    ...
    if (processClass) {
      ...
      throw new Error(
        `Process with identifier ${processName.toString()} is not registered. Make sure the process class is imported.`,
      );
    }
  }
...
export const getService = <T>(serviceName: symbol) => {
  ...
  if (!cont.isBound(serviceName)) {
    ...
    if (processClass) {
      ...
      throw new Error(
        `Service with identifier ${serviceName.toString()} is not bound to container.`,
      );
    }
  }
