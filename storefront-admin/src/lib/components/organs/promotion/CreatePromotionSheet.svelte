<script lang="ts">
	...
	let {
		...
		open?: boolean;
		campaigns: Campaign[];
		onSave?: (payload: { promotion: Promotion; newCampaign?: Campaign }) => void;
	} = $props();
	...
	function savePromotion() {
		if (detailMethod === 'promotion_code' && !detailCode.trim()) return;
		...
		// campaignChoice === 'without' -> campaignId stays null

		const newPromotion: Promotion = {
			id: crypto.randomUUID(),
			code: detailMethod === 'promotion_code' ? detailCode.trim() : '',
			method: detailMethod === 'automatic' ? 'Automatic' : 'Manual',
			status: detailStatus === 'active' ? 'Active' : 'Draft',
			campaign_id: campaignId ?? null
		};

		onSave?.(
			newCampaign != null ? { promotion: newPromotion, newCampaign } : { promotion: newPromotion }
		);
		open = false;
	}
