<script lang="ts">
	...
	async function fetchRegions() {
		try {
			const res = await fetch(`${apiBase}/regions?limit=100`, { cache: 'no-store' });
			if (res.ok) {
	...
	async function fetchCurrencies() {
		try {
			const res = await fetch(`${apiBase}/currencies?limit=100`, { cache: 'no-store' });
			if (res.ok) {
				const json = (await res.json()) as
	...
	async function addSelectedProducts() {
		...
		for (const productId of selectedProductIds) {
			...
			if (!product) continue;
			try {
				const variantsRes = await fetch(`${apiBase}/product-variants?limit=100`, { cache: 'no-store' });
				let variants: Array<{
					id: string;
					title: string;
				...
				if (variantsRes.ok) {
					const variantsData = (await variantsRes.json()) as
						| { data?: Array<{ id: string; title: string; product_id: string | null; thumbnail: string | null }> }
						| { rows?: Array<{ id: string; title: string; product_id: string | null; thumbnail: string | null }> };
					const raw = 'data' in variantsData && Array.isArray(variantsData.data)
						? variantsData.data
						: 'rows' in variantsData && Array.isArray((variantsData as { rows?: typeof variants }).rows)
				...
				if (variant) {
					try {
						const variantRes = await fetch(`${apiBase}/product-variants/${variant.id}`, { cache: 'no-store' });
						if (variantRes.ok) {
	...
	async function fetchCustomersForOrder() {
		...
		try {
			...
			if (customerBrowserSearch.trim()) params.append('search', customerBrowserSearch.trim());
							...
							{#if selectedCustomer}
								<div class="flex flex-col gap-3">
									...
									<div class="space-y-1 text-sm">
										<div>
											<button type="button" class="text-muted-foreground hover:text-foreground hover:underline">
							...
							{#if selectedTagsList.length > 0}
								<div class="mt-2 flex flex-wrap gap-1.5">
									{#each selectedTagsList as label}
										<span class="inline-flex items-center gap-1 rounded-md border bg-muted/50 px-2 py-1 text-sm text-muted-foreground">
											{label}
											<button
												type="button"
...
<Dialog.Root bind:open={productBrowserOpen}>
	<Dialog.Content class="max-w-3xl h-auto max-h-[85vh] m-auto flex flex-col rounded-xl border shadow-lg">
						...
						<table class="w-full text-sm">
							...
							<tbody>
								{#if productBrowserProducts.length === 0}
									<tr>
										<td colspan="3" class="px-4 py-8 text-center text-muted-foreground">No products found.</td>
									</tr>
								{:else}
									{#each productBrowserProducts as product (product.id)}
										...
										>
											...
											<td class="px-4 py-3">
												<div class="flex items-center gap-3">
													{#if product.thumbnail}
														<img src={product.thumbnail} alt="" class="size-10 shrink-0 rounded-md object-cover" />
													{:else}
														<div class="flex size-10 shrink-0 items-center justify-center rounded-md bg-muted text-muted-foreground">
															<ImageIcon class="size-5" />
														</div>
											...
											<td class="px-4 py-3 text-muted-foreground">
												<span class={statusBadgeClass(product.status)}>{product.status.replace(/_/g, ' ')}</span>
											</td>
										</tr>
									{/each}
								{/if}
				...
				<div class="flex items-center justify-between gap-4">
					<div class="flex items-center gap-4">
						<div class="flex items-center gap-2">
							<Button variant="outline" size="sm" disabled={!productBrowserPagination?.has_previous_page} onclick={() => (productBrowserPage = productBrowserPage - 1)}>Prev</Button>
							<span class="text-sm text-muted-foreground">{productBrowserPagination?.page ?? 1} of {productBrowserPagination?.total_pages ?? 1} pages</span>
							<Button variant="outline" size="sm" disabled={!productBrowserPagination?.has_next_page} onclick={() => (productBrowserPage = productBrowserPage + 1)}>Next</Button>
						</div>
		...
		<div class="flex flex-1 flex-col overflow-hidden">
			...
			<div class="flex flex-wrap items-center justify-between gap-4 border-b px-6 py-4">
				<div class="relative w-64 ml-auto">
					<Search class="absolute left-3 top-1/2 size-4 -translate-y-1/2 text-muted-foreground" />
					<Input type="search" placeholder="Search customers" bind:value={customerBrowserSearch} class="h-9 rounded-md pl-9" />
				</div>
			</div>
			<div class="flex flex-1 flex-col overflow-auto p-6">
						...
						<table class="w-full text-sm">
							...
							<tbody>
								{#if customerBrowserCustomers.length === 0}
									<tr>
										<td colspan="3" class="px-4 py-8 text-center text-muted-foreground">No customers found.</td>
									</tr>
								{:else}
			...
			<Dialog.Footer class="flex flex-wrap items-center justify-between gap-4 border-t px-6 py-4">
				<div class="flex items-center gap-4">
					<div class="flex items-center gap-2">
						<Button variant="outline" size="sm" disabled={customerBrowserPageNum <= 1} onclick={() => (customerBrowserPage = customerBrowserPageNum - 1)}>Prev</Button>
						<span class="text-sm text-muted-foreground">{customerBrowserPageNum} of {customerBrowserTotalPages} pages</span>
						<Button variant="outline" size="sm" disabled={customerBrowserPageNum >= customerBrowserTotalPages} onclick={() => (customerBrowserPage = customerBrowserPageNum + 1)}>Next</Button>
					</div>
					<p class="text-sm text-muted-foreground">
...
<Dialog.Root bind:open={markAsPaidModalOpen}>
	<Dialog.Content class="max-w-md h-auto max-h-[85vh] top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full flex flex-col rounded-xl border shadow-lg p-0">
		...
		<Dialog.Footer class="px-6 py-4 border-t !flex-row justify-end gap-2">
			<Button variant="outline" onclick={() => (markAsPaidModalOpen = false)} disabled={creatingOrder}>Cancel</Button>
	...
	<Sheet.Content side="right" class="w-full max-w-lg sm:max-w-lg">
		<div class="flex h-full flex-col">
				...
				<div class="flex flex-col gap-6">
						...
						<div class="flex flex-col gap-3">
							<div class="flex flex-col gap-2">
								...
								<Select.Root type="single" value={billingCountry} onValueChange={(v) => v && (billingCountry = v)}>
									<Select.Trigger class="h-10 w-full" id="billing-country">{billingCountry}</Select.Trigger>
									<Select.Content>
							...
							<div class="grid grid-cols-2 gap-4">
								...
								<div class="flex flex-col gap-2">
									...
									<Select.Root type="single" value={billingState || undefined} onValueChange={(v) => (billingState = v ?? '')}>
										<Select.Trigger class="h-10 w-full" id="billing-state">{billingState || 'Select a state'}</Select.Trigger>
										<Select.Content>
							...
							<div class="flex flex-col gap-2">
								...
								<div class="flex gap-2">
									<Select.Root type="single" value={billingPhoneCode} onValueChange={(v) => v && (billingPhoneCode = v)}>
										<Select.Trigger class="h-10 w-[100px] shrink-0">{billingPhoneCode}</Select.Trigger>
										<Select.Content>
					...
					<div class="flex flex-col gap-2">
						...
						<div class="relative">
							...
							<Input id="card-number" type="text" placeholder="1234 5678 9012 3456" class="h-10 pl-10" />
						</div>
					</div>
					<div class="grid grid-cols-2 gap-4">
						<div class="flex flex-col gap-2">
			...
			<Sheet.Footer class="flex justify-end gap-2 border-t p-4">
				<Button variant="outline" onclick={() => (creditCardSheetOpen = false)} disabled={creatingOrder}>Cancel</Button>
				<Button disabled={creatingOrder || orderItems.length === 0} onclick={() => createOrder()} class="flex items-center gap-2">
					{#if creatingOrder}
...
<Dialog.Root bind:open={notesModalOpen}>
	<Dialog.Content class="max-w-md h-auto max-h-[85vh] top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full flex flex-col rounded-xl border shadow-lg p-0">
		...
		<Dialog.Footer class="px-6 py-4 border-t !flex-row justify-end gap-2">
			<Button variant="outline" onclick={() => (notesModalOpen = false)}>Cancel</Button>
			<Button onclick={() => (notesModalOpen = false)}>Save</Button>
		</Dialog.Footer>
	</Dialog.Content>
</Dialog.Root>
...
<Dialog.Root bind:open={addTagsModalOpen}>
	<Dialog.Content class="max-w-lg h-auto max-h-[85vh] top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full flex flex-col rounded-xl border shadow-lg p-0">
		...
		<div class="flex flex-1 flex-col overflow-hidden px-6 py-4">
			<div class="flex items-center gap-2">
				<div class="relative flex-1">
					<Search class="absolute left-3 top-1/2 size-4 -translate-y-1/2 text-muted-foreground pointer-events-none" />
					<Input type="search" placeholder="Search to find or create tags." bind:value={tagSearch} class="h-10 rounded-md border-primary pl-9" />
				</div>
				<button type="button" class="flex h-10 shrink-0 items-center justify-center gap-2 rounded-md border px-3 text-muted-foreground transition-colors hover:bg-muted hover:text-foreground" aria-label="Frequently used">
			...
			<div class="mt-4">
				...
				<div class="max-h-64 space-y-2 overflow-auto rounded-md border p-2">
					{#each filteredTags as { label, i }}
						<label class="flex cursor-pointer items-center gap-2 rounded-sm py-1.5 px-2 hover:bg-muted/50">
							<input type="checkbox" checked={selectedTagIds.has(i)} onchange={() => toggleTag(i)} class="size-4 rounded border-input" />
		...
		<Dialog.Footer class="flex flex-row justify-end gap-2 border-t px-6 py-4">
			<Button variant="outline" onclick={() => (addTagsModalOpen = false)}>Cancel</Button>
			<Button onclick={saveTagsModal}>Save</Button>
		</Dialog.Footer>
	</Dialog.Content>
</Dialog.Root>
