...
const CollectionDeleteSchema = z.object({
	collection_ids: z.array(z.string()).min(1, 'Collection IDs are required')
});
...
export const actions = {
	...
	delete: async ({ request }) => {
		const collectionDeleteForm = await superValidate(request, zod4(CollectionDeleteSchema));
		if (!collectionDeleteForm.valid) {
			return fail(400, { collectionDeleteForm });
		}
		const res = await client['collections'].delete({
			collection_ids: collectionDeleteForm.data.collection_ids
		});
		if (res.error) {
			return fail(400, { error: 'Failed to delete collections' });
		}
		return message(collectionDeleteForm, 'Collections deleted successfully');
	}
} satisfies Actions;
