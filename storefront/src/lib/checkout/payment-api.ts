...
export type StripePaymentElementSession = {
	transactionId: string;
...
export function savePendingStripePayment(
  ...
  if (!browser) return;
  const payload: PendingStripePayment = { orderId, paymentId, transactionId };
...
export async function fetchStripePublishableKey(): Promise<string> {
  const res = await client.storefront['stripe-config'].get();
  if (res.error) throw new Error(treatyErrorMessage(res.error));
  const key = (res.data as { publishable_key?: string } | undefined)?.publishable_key;
  if (!key) throw new Error('Stripe publishable key was not returned');
  return key;
}
...
export async function startStripePaymentElement(
  paymentId: string
): Promise<StripePaymentElementSession> {
  ...
  if (!data.payment_intent_client_secret) {
    throw new Error('Payment intent client secret was not returned');
  }
	return {
