<script lang="ts">
    ...
    import {
        getCheckoutPath,
        hasCheckoutStepParam,
        parseCheckoutStepFromSearchParams,
        type CheckoutStep,
    } from "$lib/checkout/checkout-url";
    import {
        checkoutFormFromCartShipping,
        clearCheckoutDraft,
        loadCheckoutDraft,
        saveCheckoutDraft,
    ...
    let { data, routeCartId = null }: { data: CheckoutPageData; routeCartId?: string | null } =
        $props();
    ...
    let formHydratedForCartId = $state<string | null>(null);
    let draftSaveTimer: ReturnType<typeof setTimeout> | null = null;
    const activeCartId = $derived(routeCartId ?? cartState.cart?.id ?? null);

    function navigateToStep(step: CheckoutStep, replaceState = false) {
        currentStep = step;
        void goto(getCheckoutPath(step, activeCartId), { replaceState });
    }

    $effect(() => {
        if (!browser) return;
        void initCartState(false, routeCartId || undefined);
    });
    ...
    $effect(() => {
        ...
        goto(getCartPagePath(activeCartId));
    });

    $effect(() => {
        if (!browser) return;
        isLoggedIn = hasCustomerAuthSession();
        if (!hasCheckoutStepParam(page.url.searchParams)) {
            goto(getCheckoutPath("addresses", activeCartId), { replaceState: true });
            return;
        }
        const stepFromUrl = parseCheckoutStepFromSearchParams(page.url.searchParams);
    ...
    function scheduleCheckoutDraftSave(cartId: string, values: CheckoutFormData) {
        if (draftSaveTimer) clearTimeout(draftSaveTimer);
        draftSaveTimer = setTimeout(() => {
            saveCheckoutDraft(cartId, values);
            draftSaveTimer = null;
        }, 300);
    }

    $effect(() => {
        if (!browser || !cartState.initialized || cartState.loading) return;
        const cartId = activeCartId;
        if (!cartId || formHydratedForCartId === cartId) return;

        const draft = loadCheckoutDraft(cartId);
        const fromCart = checkoutFormFromCartShipping(cartState.cart);
        ...
        form.update((f) => ({
            ...
            paymentMethod:
                draft?.paymentMethod ??
                fromCart?.paymentMethod ??
                current.paymentMethod,
            shippingMethod:
                draft?.shippingMethod ??
                fromCart?.shippingMethod ??
                current.shippingMethod,
        }));
        formHydratedForCartId = cartId;
    });

    $effect(() => {
        if (!browser || !cartState.initialized) return;
        const cartId = activeCartId;
        if (!cartId || formHydratedForCartId !== cartId) return;

        const unsub = form.subscribe((values) => {
            scheduleCheckoutDraftSave(cartId, values);
        });
        return () => {
            unsub();
            if (draftSaveTimer) {
    ...
    function goBack() {
        if (currentStep === "delivery") navigateToStep("addresses");
        else if (currentStep === "payment") navigateToStep("delivery");
        else if (currentStep === "review") navigateToStep("payment");
    }
    ...
    async function placeOrder() {
        ...
        try {
            ...
            if (useStripeCheckout && selectedProvider) {
                ...
                clearCheckoutDraft(readyCartId);
                await goto(
