...
async function fetchProductByHandle(
  ...
  const res2 = await fetch(`${API_BASE}/products?${fallbackParams}`, {
    cache: "no-store",
  });
...
export async function load({ params }) {
  ...
  try {
    ...
    const variantsWithPrices: Array<ApiVariant & { priceDisplay: string }> =
      await Promise.all(
        variants.map(async (v) => {
          try {
            const r = await fetch(`${API_BASE}/product-variants/${v.id}`, {
              cache: "no-store",
            });
            ...
            const priceDisplay =
              p.currency_code === "USD"
                ? `$${amount.toFixed(2)}`
                : `${p.currency_code.toUpperCase()} ${amount.toFixed(2)}`;
    ...
    const listRes2 = await fetch(`${API_BASE}/products?limit=20&page=1`, {
      cache: "no-store",
    });
    ...
    const otherProducts = await Promise.all(
      others.map(async (p, i) => {
        ...
        if (vId) {
          try {
            const r = await fetch(`${API_BASE}/product-variants/${vId}`, {
              cache: "no-store",
            });
            if (r.ok) {
              ...
              if (pr) {
                ...
                priceStr =
                  pr.currency_code === "USD"
                    ? `$${amt.toFixed(2)}`
                    : `${pr.currency_code.toUpperCase()} ${amt.toFixed(2)}`;
              }
            }
          } catch {}
        }
