<script lang="ts">
    ...
    let detailByOrderId = $state(new Map<string, OrderDetail>());
    ...
    const totalPages = $derived.by(() =>
        Math.max(1, Math.ceil(sortedOrders.length / ORDERS_PER_PAGE)),
    );
    ...
    const hasPreviousPage = $derived.by(() => currentPage > 1);
    const hasNextPage = $derived.by(() => currentPage < totalPages);
    const rangeStart = $derived.by(() =>
        sortedOrders.length === 0 ? 0 : (currentPage - 1) * ORDERS_PER_PAGE + 1,
    );
    ...
    const detailForSummary = (summary: OrderSummary): OrderDetail =>
        detailByOrderId.get(summary.id) ?? loadOrderDetails(summary);

    const lineItemsForOrder = (summary: OrderSummary): OrderDetail["items"] => {
        const detail = detailForSummary(summary);
    ...
    $effect(() => {
        ...
        const loaded = parseStoredOrders(localStorage.getItem(ordersKey));
        orders = loaded;
        currentPage = 1;

        let cancelled = false;
        void (async () => {
            const next = new Map<string, OrderDetail>();
            for (const summary of loaded) {
                const cached = loadOrderDetails(summary);
                if (cached.items.length > 0) {
                    next.set(summary.id, cached);
                    continue;
                }
                const ref = summary.orderId || summary.id;
                const fetched = await fetchOrderDetailFromApi(ref);
                if (fetched) next.set(summary.id, fetched);
            }
            if (!cancelled) detailByOrderId = next;
        })();
        ...
        {:else}
            <div class="orders-feed">
                {#each paginatedOrders as order (order.id)}
                    {@const detail = detailForSummary(order)}
                    ...
                    <article class="order-amazon-card">
                        <header class="order-amazon-card__meta">
                            <div class="order-meta-block">
                                ...
                                <strong class="order-meta-value"
                                    >{formatLongDate(order.date)}</strong
                                >
                            </div>
                            <div class="order-meta-block">
                                <span class="order-meta-label">Total</span>
                                <strong class="order-meta-value"
                                    >{formatStoreMoney(order.total)}</strong
                                >
                            </div>
                            <div
                                class="order-meta-block order-meta-block--ship"
                            >
                            ...
                            <div
                                class="order-meta-block order-meta-block--actions"
                            >
                                <p class="order-meta-order-id">
                                    <span class="order-meta-label">Order #</span
                                    >
                                    <span class="order-meta-order-number"
                                        >{displayOrderNumber(
                                            detail,
                                            order,
                                        )}</span
                                ...
                                <p class="order-meta-links">
                                    ...
                                    >
                                    <span
                                        class="order-action-sep"
                                        aria-hidden="true">|</span
                                    >
                                    <button
                                        type="button"
                                        class="order-link-btn order-link-btn--caret"
                                    >
                                        Invoice
                                        <svg
                                            width="10"
                        ...
                        <div class="order-amazon-card__body">
                            ...
                            <aside
                                class="order-amazon-card__aside"
                                aria-label="Order actions"
                            >
                                <button
                                    type="button"
                                    class="order-pill-btn order-pill-btn--full"
                                        ...
                                        <div class="order-line__content">
                                            {#if item.productHandle}
                                                <a
                                                    ...
                                                    class="order-line__title"
                                                    >{item.title}</a
                                                >
                                            {:else}
                                                <p
                                                    class="order-line__title order-line__title--plain"
                                                >
                                                    {item.title}
                                                </p>
                                            {/if}
                                            {#if item.variant}
                                                <p class="order-line__variant">
                                                    {item.variant}
                                                </p>
                                            {/if}
                                            <p class="order-line__return">
                                                {returnWindowText(detail)}
                                            </p>
                                            <div class="order-line__cta">
                                                <a
                                                    href="/store"
                                                    class="order-pill-btn order-pill-btn--primary"
                                                    >Buy it again</a
                                                >
                                                <a
                                                    href="/store"
                                                    class="order-pill-btn"
                                                    >View your item</a
                                                >
                                            </div>
                                        </div>
            ...
            {#if sortedOrders.length > ORDERS_PER_PAGE}
                <nav class="orders-pagination" aria-label="Orders pagination">
                    <span class="orders-pagination__range">
                        {rangeStart}–{rangeEnd} of {sortedOrders.length}
                    </span>
